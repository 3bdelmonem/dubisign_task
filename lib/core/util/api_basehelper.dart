...
class ApiBaseHelper {
  ...
  Future<dynamic> get({required String url, String? token}) async {
    try {
      ...
      log(baseUrl + url);
      final Response response = await dioInit(dio).get(baseUrl + url);
      ...
      if (e.response!.statusCode == 401) {
        // sl<AppCubit>().unAuthenticate();

        // sl<AppNavigator>().popUtill(screen: const LoginScreen());
      }
  ...
  dynamic _returnResponse(Response response) {
    switch (response.statusCode) {
      ...
      case 500:
      default:
        log('Error occurred while Communication with Server with StatusCode : ${response.statusCode} ${response.data}');
        throw FetchDataException('Error occurred while Communication with Server with StatusCode : ${response.statusCode} ${response.data}');
    }
  }
}
